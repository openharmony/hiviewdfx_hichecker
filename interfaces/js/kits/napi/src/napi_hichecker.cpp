...
uint64_t GetRuleParam(napi_env env, napi_callback_info info)
{
    ...
    if (argc != ONE_VALUE_LIMIT) {
        HILOG_ERROR(LABEL, "invalid number=%{public}d of params.", ONE_VALUE_LIMIT);
        return GET_RULE_PARAM_FAIL;
    }
    if (!MatchValueType(env, argv[ARRAY_INDEX_FIRST], napi_bigint)) {
        HILOG_ERROR(LABEL, "Type error, should be bigint type!");
        return GET_RULE_PARAM_FAIL;
    }
    ...
    if (!lossless) {
        HILOG_ERROR(LABEL, "Type error, bigint should be 64!");
        return GET_RULE_PARAM_FAIL;
    }
    if (rule == GET_RULE_PARAM_FAIL) {
        HILOG_ERROR(LABEL, "invalid input, please check!");
    }
    return rule;
}
